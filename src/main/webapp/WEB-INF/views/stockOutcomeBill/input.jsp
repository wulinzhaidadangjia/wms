...
<head>
    ...
    <script>
        $(function () {
                ...
                $.artDialog.open("/product/selectProductList.do", {
                    ...
                    close: function () {
                        ...
                        if (productinfo) {
                            ...
                            tr.find("[tag=salePrice]").val(productinfo.salePrice);
                            tr.find("[tag=number]").val(1);
                            tr.find("[tag=amount]").text(productinfo.salePrice.toFixed(2));
                        }
                    }
                });
            });
            ...
            //计算总金额
            $("[tag=number],[tag=salePrice]").blur(function () {
                var currentTr = $(this).closest("tr");
                var totalNumer = currentTr.find("[tag=number]").val();
                var salePrice = currentTr.find("[tag=salePrice]").val();
                currentTr.find("[tag=amount]").text((totalNumer * salePrice).toFixed(2));
            });
            //保存弹出提示
